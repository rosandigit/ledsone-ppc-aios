...
| GAP-C09 | Hour Budget Rules — unresolved Known Limitations | `context/hour-budget-rules.md` | Owning file → "Known Limitations" | Open `[VERIFY]` | Jathukulan / [VERIFY] | [VERIFY] | [VERIFY] | Canonical intraday hourly budget-boost rule; a distinct mechanism from the daily budget rules in `context/budget-rules.md` (see GAP-C07). Unresolved `[VERIFY]` items are owned in the file's Known Limitations and indexed here at pointer level only, not restated. Owning file records Owner: Amazon PPC AIOS, Reviewer: Jathukulan, Status: Draft. |
| GAP-C10 | Product Pause Rules — unresolved Known Limitations | `context/product-pause-rules.md` | Owning file → "Known Limitations" | Open `[VERIFY]` | Jathukulan / [VERIFY] | [VERIFY] | [VERIFY] | Canonical product-performance pause decision logic. Overlaps but does **not** resolve GAP-C07 (non-UK Spend Pause values in `context/budget-rules.md`) or GAP-C08 (vendor bridge / product data), which remain open. Unresolved `[VERIFY]` items are owned in the file's Known Limitations and indexed here at pointer level only, not restated. Owning file records Owner: Amazon PPC AIOS, Reviewer: Jathukulan, Status: Draft. |
...
| GAP-E02 | Evidence retention policy — partly approved; archive trigger undefined | `evidence/README.md` | Owning file → "Evidence Retention Governance" and "Known Limitations" | Open `[VERIFY]` | Jathukulan / [VERIFY] | [VERIFY] | [VERIFY] | Owning file now records approved retention governance for confidential primary evidence in the approved non-Git architecture. Remains open: archive trigger, preservation exception, and retention for evidence outside that scope. Values are not restated here — see owning file. |
